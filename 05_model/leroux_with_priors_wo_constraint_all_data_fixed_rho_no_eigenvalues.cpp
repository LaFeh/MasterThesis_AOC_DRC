...
Type objective_function<Type>::operator()() {
  ...
  Type rho = 0.6;//invlogit(logit_rho);
  ...
  nll -= dgamma(tau, tau_prior_shape, tau_prior_scale, true) +log(tau);
  //nll -= dnorm(logit_rho, logit_rho_prior_mean, logit_rho_prior_sd, true);
  
  // ── Likelihood (observed areas; NAs pre-filled with y=0,n=0 upstream) ────
