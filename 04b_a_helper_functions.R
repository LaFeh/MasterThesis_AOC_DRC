...
prepare_adj_matrix_for_prediction <- function(frontline_data,
  ...
  if(!second_degree_neighbours) {
    ...
    data_adj_second = mat2listw(data_mat_2,zero.policy = T,style ="B")
  ...
  if (!second_degree_neighbours) {
    ...
    for (start_point in seq_along(data_adj_second$neighbours)) {
      ...
      for (end_point_idx in seq_along(all_neighbours)) {
        ...
        # if (end_point %in% first_deg_sp) {
        # 
        #   m <- (decay[start_point] + decay[end_point]) / 2
        # 
        #   if (bol_distance) {
        #     dist <- euclid_dist(start_point, end_point)
        #     if (dist < 1) dist <- 1
        #     weights[end_point_idx] <- 1 / (m * dist)
        #   } else {
        #     weights[end_point_idx] <- 1 / m
        #   }
        # 
        # } else {
          ...
          if (bol_distance) {
            dist <- euclid_dist(start_point, end_point)
            if (dist < 1) dist <- 1
            weights[end_point_idx] <- 1 / sqrt(m * (dist/1000))
          } else {
    ...
    if (bol_distance){
      # scaling_std = sqrt(var(unlist(weights_second_neightbours_decay)))
      # weights_second_neightbours_decay = sapply(weights_second_neightbours_decay,
      #                                           function(x){
      #                                             result = x/scaling_std; if(length(result)>0 & result>1){return(1)}else{result}})
      # 
