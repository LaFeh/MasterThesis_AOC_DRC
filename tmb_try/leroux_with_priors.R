...
# ── 1. Clean recompile ────────────────────────────────────────────────────────
try(dyn.unload(dynlib("./tmb_try/leroux_with_priors")), silent = TRUE)
file.remove("./tmb_try/leroux_with_priors.o")
file.remove("./tmb_try/leroux_with_priors.dll")

compile("./tmb_try/leroux_with_priors.cpp")
dyn.load(dynlib("./tmb_try/leroux_with_priors"))
...
data_lst <- list(
  ...
  sum_to_zero_kappa = 0.1/N,
  beta_prior_sd        = 2.5,
  log_tau_prior_mean   = 1,
  log_tau_prior_sd     = 5e-04,
  logit_rho_prior_mean = 0,
  logit_rho_prior_sd   = 2
)
...
save(rep,file = "./tmb_try/leroux_with_priors_mat_w.RData")


mu <- rep$par.fixed
Sigma <- rep$cov.fixed

idx <- match(c("log_tau", "logit_rho"), names(mu))

set.seed(1)
draws_fixed <- MASS::mvrnorm(
  n = 10000,
  mu = mu,
  Sigma = Sigma
)
...
draws <- data.frame(
  log_tau   = draws_fixed[, idx[1]],
  logit_rho = draws_fixed[, idx[2]]
)

draws$tau <- exp(draws$log_tau)
draws$rho <- plogis(draws$logit_rho)
p

hist(draws$tau,
    # breaks = 50,
     main = "Approximate distribution of tau",
     xlab = "tau")

hist(draws$rho, breaks = 50, main = "Approximate distribution of rho",
     xlab = "rho",col ="orange")
plot(draws$rho, draws$tau,
     xlab = "rho",
     ylab = "tau",
     main = "Approximate joint distribution of rho and tau")
...
data$observed <- !is.na(data$control_binom)
