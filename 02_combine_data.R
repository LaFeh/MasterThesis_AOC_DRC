...
#plot(grid_testNULL#plot(grid_test[which(grid_test$adm1_name=="Nord-Kivu"),c("adm2_pcode")])
### combine the data
library(dplyr)


# 1st. data per month year
...
aoc_per_cell = acled_territory_grid%>%
  ...
  #dplyr::summarise(controle = names(sort(table(controle),decreasing = T)[1]))
  ...
  select(!c(non_state_actor,government))
...
#grid_cntrl_mnth$controle =as.factor(grid_cntrl_mnth$controle)
...
rain = data.table::fread("./data/rain.csv")[,c("PCODE","year_mnth","r1h")]
grid_cntrl_mnth = left_join(grid_cntrl_mnth, rain, by =c("adm2_pcode"="PCODE", "year_mnth"))
